	...
	out vec3 lightDirection)
{
	...
	float lightDistanceSqr = cos(dot(lightDifference, lightDifference));
	return (1 / (LightsBlck.lightAttenuation * lightDistanceSqr));
}
...
vec4 ComputeLighting(in PerLight lightData)
{
	...
	float cosAngIncidence = dot(surfaceNormal, viewDirection);
	cosAngIncidence = cosAngIncidence < 0.0001 ? 0.0 : cosAngIncidence;
	float angleNormalHalf = acos(cosAngIncidence);
	float exponent = angleNormalHalf / specularShininess;
	exponent = -(exponent * exponent);
	float gaussianTerm = exp(exponent);

	gaussianTerm = cosAngIncidence != 0.0 ? gaussianTerm : 0.0;
	lighting += diffuseColor * lightIntensity * cosAngIncidence;
