...
IntersectionData IntersectionAlgorithms::RayYPlaneIntersect(Ray& ray, float yPos)
{
	IntersectionData intersectionData;
	intersectionData.t = -1;

	if(IntersectionAlgorithms::equal(ray.direction.y,0))
		return intersectionData;

	float t = (yPos-ray.origin.y)/ray.direction.y;
	glm::vec3 point = ray.GetPointAlongRay(t);
